...
function form_save()
{
    ...
    if ((isset_request_var('save_component_data')) && (!is_error_message())) {
        ...
        $input_fields = db_fetch_assoc_prepared(
            "SELECT dtd.data_input_id, dl.host_id, dif.id, dif.input_output,
            ...
            foreach ($input_fields as $input_field) {
                if (isset_request_var('value_' . $input_field['id'])) {
                    ...
                    $data_template_id = db_fetch_cell_prepared(
                        'SELECT local_data_template_data_id
						FROM data_template_data
						WHERE id = ?',
                        array(get_request_var('data_template_data_id'))
                    );

                    $is_templated = db_fetch_cell_prepared(
                        'SELECT t_value
            ...
            if (!isempty_request_var('local_data_id') || isempty_request_var('data_template_id')) {
                ...
                if (cacti_sizeof($rrds)) {
                    foreach ($rrds as $rrd) {
                        ...
                        $save3['local_data_template_rrd_id'] = db_fetch_cell_prepared(
                            'SELECT local_data_template_rrd_id
							FROM data_template_rrd
							WHERE id = ?',
                            array($rrd['id'])
                        );
...
function data_edit($incform = true)
{
    ...
    if (!empty($data['data_input_id'])) {
        ...
        $data_input_name = db_fetch_cell_prepared(
            'SELECT name
			FROM data_input
			WHERE id = ?',
            array($data['data_input_id'])
        ...
        if (cacti_sizeof($fields) > 0) {
            foreach ($fields as $field) {
                ...
                if (empty($data['data_template_id'])) {
                    ...
                    $can_template = db_fetch_cell_prepared(
                        'SELECT t_value
						FROM data_input_data
						WHERE data_template_data_id = ?
						AND data_input_field_id = ?',
...
function ds_edit()
{
    ...
    if (!isempty_request_var('id')) {
        $data_local = db_fetch_row_prepared(
            'SELECT host_id, data_template_id
			FROM data_local
			WHERE id = ?',
            array(get_request_var('id'))
        );
    ...
    if (cacti_sizeof($data_template)) {
        $data_sources = db_fetch_cell_prepared(
            'SELECT
    ...
    if (get_request_var('host_id') > 0) {
        $hostDescription = db_fetch_cell_prepared(
            'SELECT description
			FROM host
			WHERE id = ?',
    ...
    if (((isset_request_var('id')) || (isset_request_var('new'))) && (empty($data['data_template_id']))) {
        ...
        if (!isempty_request_var('view_rrd')) {
            $local_data_template_rrd_id = db_fetch_cell_prepared(
                'SELECT local_data_template_rrd_id
				FROM data_template_rrd
				WHERE id = ?',
            ...
            $rrd_template = db_fetch_row_prepared(
                'SELECT *
				FROM data_template_rrd
				WHERE id = ?',
...
function get_graphs_aggregates_url($local_data_id)
{
    $graphs = db_fetch_row_prepared(
        'SELECT GROUP_CONCAT(DISTINCT gl.id) AS graphs, COUNT(DISTINCT gl.id) AS total
		FROM data_local AS dl
		INNER JOIN data_template_rrd AS dtr
		ON dl.id = dtr.local_data_id
    ...
    $aggregates = db_fetch_row_prepared(
        'SELECT GROUP_CONCAT(DISTINCT gl.id) AS graphs, COUNT(DISTINCT gl.id) AS total
		FROM data_local AS dl
		INNER JOIN data_template_rrd AS dtr
		ON dl.id = dtr.local_data_id
