...
test('the production wait loop retries after a transient drain failure', function () {
    ...
    $program = 'const MAX_POLLER_RUNTIME=298;$poller_start=microtime(true);$calls=0;$poller_id=1;$rrds_processed=0;$poller_output_deferred=false;$rrdtool_pipe=false;'
...
test('the final drain reports current failure state after a recovered waiting attempt', function ($failed) {
    ...
    $program = 'const MAX_POLLER_RUNTIME=298;$poller_start=microtime(true);$rrds_processed=0;$poller_output_deferred=true;$rrd_write_failed=true;$rrdtool_pipe=false;'
