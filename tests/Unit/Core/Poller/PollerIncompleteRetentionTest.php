...
beforeEach(function () {
    ...
    $GLOBALS['retention_db']->sqliteCreateFunction('UNIX_TIMESTAMP', function ($value = null) { return $value === null ? strtotime('2020-01-03 UTC') : strtotime($value); }, -1);
    $GLOBALS['retention_db']->sqliteCreateFunction('FROM_UNIXTIME', function ($value) { return gmdate('Y-m-d', $value); }, 1);
...
test('expiry preserves complete groups, recent partials, boundary timestamps and concurrent arrivals', function () {
    ...
    $expired = poller_expire_incomplete_rows(86400, $failed);
...
test('unavailable database operations retain the selected samples', function ($failure) {
    ...
    expect(poller_expire_incomplete_rows(86400, $failed))->toBe(0)->and($failed)->toBeTrue()
