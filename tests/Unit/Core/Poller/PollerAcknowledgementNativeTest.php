...
test('production poller files retain failed writes and preserve concurrent arrivals', function ($realtime, $failed) {
    ...
    try {
        if ($realtime) {
            ...
            file_put_contents($dir . '/run.php', $bootstrap . 'require ' . var_export($root . '/lib/poller.php', true) . ';$pipe=true;$result=process_poller_output($pipe,1,$deferred);if(getenv("ACK_FAIL")==="rejected"){process_poller_output($pipe,1,$secondDeferred);}db_close();exit($deferred?1:0);');
...
})->with(array(array(false,false),array(false,true),array(true,false),array(true,true),array(false,'rejected'),array(true,'rejected'),array(false,'mixed'),array(false,'page')));
