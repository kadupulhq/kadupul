            ...
            if ($signal !== null) {
                ...
                while (!$opened && microtime(true) < $deadline) {
                    foreach (glob('/proc/' . $pid . '/fd/*') as $fd) {
                        ...
                        if ($target === $dir . '/store' && is_string($info) && strpos($info, 'lock:') === false) {
                            $opened = true;
                            break;
                        }
                    }
                ...
                $deadline = microtime(true) + 10;
                while (!file_exists($dir . '/unregistered') && microtime(true) < $deadline) {
                    usleep(10000);
                }
...
test('remote schema upgrades do not require local RRD storage unless explicitly checked', function ($missing, $probe, $local, $engine = 'InnoDB', $connection = 'online', $forced = false) {
    $root = dirname(__DIR__, 4);
...
})->with(array(array(true,false,false),array(false,false,false),array(true,true,false),array(false,true,false),array(true,false,true),array(false,false,true),array(false,false,false,'MEMORY'),array(false,false,true,'MEMORY'),array(false,false,false,false),array(true,false,true,'MEMORY','offline'),array(true,false,true,false,'recovery'),array(true,false,false,'MEMORY','offline'),array(true,false,false,false,'recovery'),array(false,false,false,'InnoDB','online',true)));
