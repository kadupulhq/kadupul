...
function fwrite($handle, $value) {
    if ($GLOBALS['probe_mode'] === 'throw') { throw new \RuntimeException('injected write exception'); }
    return \fwrite($handle, $value);
}
function unlink($path) {
    $GLOBALS['probe_unlinks']++;
    if ($GLOBALS['probe_mode'] === 'unlink' && $GLOBALS['probe_unlinks'] === 1) { return false; }
    return \unlink($path);
}
test('Windows probes clean owned temporary files without hiding failures', function ($mode) {
    ...
    try {
        ...
        foreach (glob($directory . '/.kadupul-write-*') as $file) { \unlink($file); }
        rmdir($directory);
