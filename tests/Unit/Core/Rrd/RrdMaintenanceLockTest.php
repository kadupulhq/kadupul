...
test('poller storage preflight notifies administrators and fails closed on untrusted shared stores', function ($mode) {
    ...
    if (in_array($mode,array('trusted-group','untrusted-group'),true)) { chmod($this->dir,0770); }
    ...
    if (in_array($mode,array('memory','unknown','unreadable'),true)) { expect($result[2][0][1])->toContain('must use InnoDB')->toContain('must not be discarded'); }
    elseif ($blocked) { expect($result[2][0][1])->toContain('rrd_maintenance_trusted_uids')->toContain('rrd_maintenance_trusted_gids'); }
...
foreach(array('cacti_has_control_chars','cacti_rrdtool_valid_path','cacti_rrdtool_valid_path_token','cacti_rrdtool_valid_ds_name','cacti_rrdtool_valid_ds_template','cacti_escapeshellarg','cacti_escapeshellcmd','cacti_version_compare','version_to_decimal','cacti_sizeof') as $name){
...
PROBE;
    ...
    if ($schemaMismatch) {
        $logs = array_values(array_filter($result[10], function ($line) { return strpos($line, 'RRD pending sample retained for retry:') !== false; }));
