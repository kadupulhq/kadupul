...
test('real synchronous and queued updates retain their samples across maintenance', function ($explicitClose, $outputFlag) {
    ...
    try {
        ...
        copy($rrd, $rrd . '.replacement');
        rename($rrd . '.replacement', $rrd);
...
test('a storage directory replaced while a writer waits is rejected', function ($exclusiveWait) {
    if (!is_dir('/proc/self/fd')) {
        $this->markTestSkipped('Linux descriptor inspection synchronizes this directory replacement test.');
    }
    $path = $this->dir . '/store';
    mkdir($path, 0700);
    $GLOBALS['config']['rra_path'] = $path;
    ...
    try {
        ...
        while (!$opened && microtime(true) < $deadline) {
            foreach (glob('/proc/' . $pid . '/fd/*') as $fd) {
                ...
                if ($target === $path && is_string($info) && strpos($info, 'lock:') === false) {
                    $opened = true;
                    break;
                }
            }
        ...
        rename($path, $this->dir . '/old-store');
        mkdir($path, 0700);
    } finally {
        rrd_maintenance_release($lock);
    }
...
test('failed writer initialization preserves normal and Boost queues before any database access', function ($mode) {
    ...
    $bootstrap .= '$config = ' . var_export(array('cacti_server_os' => 'unix', 'rra_path' => $this->dir . ($mode === 'missing' ? '/missing' : ''), 'library_path' => $root . '/lib'), true) . ';' .
        ...
        'define("RRDTOOL_OUTPUT_STDOUT", 1); $fetch = rrdtool_function_fetch(1, 1700000000, 1700000060, 60, false, __DIR__ . "/source.rrd");' .
...
test('Boost archive cleanup retains samples and refuses unverifiable counts', function () {
    $root = dirname(__DIR__, 4);
    $bootstrap = '<?php ';
    if ($this->getTestResultObject()->getCodeCoverage() !== null) {
        $this->expectedChildReports = 1;
    ...
    $bootstrap .= '$counts = array(false, null, "", 0, "0", 3, 0.5); $queries = array();' .
        'function db_fetch_cell($sql) { $GLOBALS["queries"][] = $sql; return array_shift($GLOBALS["counts"]); }' .
        'require ' . var_export($root . '/lib/boost.php', true) . '; $results = array();' .
        'for ($i=0; $i<7; $i++) { $results[] = boost_archive_is_empty("poller_output_boost_arch_123"); }' .
        '$results[] = boost_archive_is_empty("poller_output_boost_arch_123; DROP TABLE settings");' .
        'echo json_encode(array($results, $queries));';
    file_put_contents($this->dir . '/archive-count.php', $bootstrap);
    $process = proc_open(array(PHP_BINARY, '-d', 'pcov.directory=' . $root, '-d', 'pcov.exclude=~/(include/vendor|tests)/~', $this->dir . '/archive-count.php'), array(1 => array('pipe', 'w'), 2 => array('pipe', 'w')), $pipes);
    $stdout = stream_get_contents($pipes[1]);
    $stderr = stream_get_contents($pipes[2]);
    fclose($pipes[1]);
    fclose($pipes[2]);
    expect(proc_close($process))->toBe(0)->and($stderr)->toBe('');
    $result = json_decode($stdout, true);
    expect($result[0])->toBe(array(false, false, false, true, true, false, false, false))
        ->and($result[1])->toBe(array_fill(0, 7, 'SELECT COUNT(*) FROM `poller_output_boost_arch_123`'));
});
...
test('a broken exclusive rewrite pipe aborts without retrying a stale snapshot', function () {
    if (!function_exists('pcntl_waitpid')) {
        $this->markTestSkipped('pcntl is required to synchronize a real broken pipe.');
    }
    $root = dirname(__DIR__, 4);
    $bootstrap = '<?php ';
    if ($this->getTestResultObject()->getCodeCoverage() !== null) {
...
test('one persistent process continues past rejected files and preserves timestamp ordering', function () {
    ...
    $bootstrap .= '$config=' . var_export(array('cacti_server_os' => 'unix', 'rra_path' => $this->dir, 'is_web' => false, 'rrd_command_timeout' => 1), true) . ';' .
        ...
        '$pipe=rrd_init(false,false,true);$result=rrdtool_function_update($updates,$pipe,$completed);rrd_close($pipe);' .
        'echo json_encode(array($result,isset($completed[$bad]),count($completed[$good]),count(rrd_acknowledged_pipes())));';
    ...
    expect($status)->toBe(0)->and(json_decode($output, true))->toBe(array(false,false,100,0));
