...
function escape_command($command) { return $command; }
...
function stream_select(&$read, &$write, &$except, $seconds, $microseconds) {
    if (++$GLOBALS['response_selects'] === 1) { $read = array(); } else { $write = array(); }
    return 1;
}
function proc_terminate($process) { $GLOBALS['response_terminated'] = true; return true; }
