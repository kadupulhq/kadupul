...
function read_config_option($key, ...$args) { return $GLOBALS['windows_cleanup_options'][$key] ?? ''; }
...
function cacti_log(...$args) { $GLOBALS['windows_cleanup_logs'][] = $args[0]; }
function raise_message(...$args) { $GLOBALS['windows_cleanup_messages'][] = $args; }
...
function db_fetch_cell(...$args) { return array_key_exists('windows_cleanup_count', $GLOBALS) ? $GLOBALS['windows_cleanup_count'] : 1; }
function db_fetch_assoc(...$args) { return empty($GLOBALS['cleanup_read_failure']) ? array() : false; }
...
function db_execute_prepared($sql, ...$args) { $GLOBALS['windows_cleanup_queries'][] = $sql; return true; }
function db_execute($sql, ...$args) { return db_execute_prepared($sql); }
function rrdclean_error_handler(...$args) { return false; }
function set_error_handler($callback) { return \set_error_handler(__NAMESPACE__ . '\\' . $callback); }
...
function with_policy($platform, $remote, $operation)
{
    ...
    try { $operation(); } finally { $GLOBALS['config'] = $saved; error_reporting($reporting); }
}
...
test('data-source deletion enqueues only supported automatic cleanup', function ($multiple, $platform, $remote, $supported) {
    with_policy($platform, $remote, function () use ($multiple, $supported) {
        ...
        if (!$supported) { expect(implode(' ', $GLOBALS['windows_cleanup_logs']))->toContain('retained for manual cleanup'); }
    });
})->with(array(
...
test('Windows maintenance skips the unsupported queue without reporting a transient failure', function () {
    with_policy('win32', 0, function () {
        ...
        } finally { unset($GLOBALS['windows_notification_available']); }
...
test('cleanup fails closed on unreadable counts before platform policy', function ($count, $platform) {
    ...
    } finally { unset($GLOBALS['windows_cleanup_count']); }
