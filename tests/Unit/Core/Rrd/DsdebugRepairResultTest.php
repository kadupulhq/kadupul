...
{
    return array('info' => array('rrd_match_array' => array('tune' => array('fixture --minimum value:0'))));
}
function cacti_sizeof($value)
{
    return count($value);
}
function cacti_unserialize($value)
{
    return $value;
}
function get_data_source_path(...$args)
{
    return $GLOBALS['repair_file'];
}
function read_config_option($key)
{
    return 'rrdtool';
}
function rrdtool_execute(...$args)
{
    return $GLOBALS['repair_result'];
}
function cacti_log($message, ...$args)
{
    $GLOBALS['repair_logs'][] = $message;
}
