...
function read_config_option($key) { return $GLOBALS['field_fixture']; }
function get_data_source_path(...$args) { return $GLOBALS['field_fixture'] . '/source.rrd'; }
function cacti_rrdtool_valid_path($path) { return true; }
function db_fetch_assoc_prepared($sql, $params) {
    if (strpos($sql, 'SELECT DISTINCT') !== false) { return false; }
    return array(array('local_data_id' => 8, 'output' => 'metric:42', 'time' => '2026-09-15 00:00:00', 'rrd_name' => 'value'));
}
function db_execute_prepared(...$args) { throw new \RuntimeException('Unprepared realtime sample was deleted'); }
function rrdtool_function_update(...$args) { throw new \RuntimeException('Failed field mapping was converted to an RRD update'); }
