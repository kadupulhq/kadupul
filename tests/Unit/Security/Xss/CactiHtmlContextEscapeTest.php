...
// CACTI_ESC_ELEMENT — HTML element content
it('CACTI_ESC_ELEMENT — HTML element content — escapes the five HTML metacharacters', function () {
	$out = cacti_html_context_escape('<script>alert(1)</script>', CACTI_ESC_ELEMENT);
	expect($out)->toBe('&lt;script&gt;alert(1)&lt;/script&gt;');
});

it('CACTI_ESC_ELEMENT — HTML element content — escapes both quote styles', function () {
	expect(cacti_html_context_escape('a"b\'c', CACTI_ESC_ELEMENT))
		->toBe('a&quot;b&#039;c');
});

it('CACTI_ESC_ELEMENT — HTML element content — escapes ampersand', function () {
	expect(cacti_html_context_escape('a&b', CACTI_ESC_ELEMENT))
		->toBe('a&amp;b');
});

it('CACTI_ESC_ELEMENT — HTML element content — preserves UTF-8 multi-byte characters', function () {
	expect(cacti_html_context_escape('café', CACTI_ESC_ELEMENT))->toBe('café');
});

// CACTI_ESC_ATTR — HTML attribute value
it('CACTI_ESC_ATTR — HTML attribute value — escapes both quote delimiters', function () {
	$out = cacti_html_context_escape('a" onmouseover="evil()"', CACTI_ESC_ATTR);
	expect($out)->not->toContain('"')
		->and($out)->toContain('&quot;');
});

it('CACTI_ESC_ATTR — HTML attribute value — blocks single-quote breakout', function () {
	$out = cacti_html_context_escape("a' onmouseover='evil()'", CACTI_ESC_ATTR);
	expect($out)->not->toContain("'")
		->and($out)->toContain('&#039;');
});

it('CACTI_ESC_ATTR — HTML attribute value — blocks angle-bracket attribute breakout', function () {
	$out = cacti_html_context_escape('a><script>alert(1)</script>', CACTI_ESC_ATTR);
	expect($out)->not->toContain('<')
		->and($out)->not->toContain('>');
});

// CACTI_ESC_JS_STRING — JavaScript string literal
it('CACTI_ESC_JS_STRING — JavaScript string literal — escapes double quote to prevent string breakout', function () {
	$out = cacti_html_context_escape('";alert(1);//', CACTI_ESC_JS_STRING);
	expect($out)->not->toContain('"');
});

it('CACTI_ESC_JS_STRING — JavaScript string literal — escapes single quote (for single-quoted JS literals)', function () {
	$out = cacti_html_context_escape("';alert(1);//", CACTI_ESC_JS_STRING);
	expect($out)->not->toContain("'");
});

it('CACTI_ESC_JS_STRING — JavaScript string literal — escapes backslash to prevent escape-sequence injection', function () {
	$out = cacti_html_context_escape('\\', CACTI_ESC_JS_STRING);
	expect($out)->toBe('\\\\');
});

it('CACTI_ESC_JS_STRING — JavaScript string literal — escapes angle brackets so <script> / </script> cannot close a block', function () {
	$out = cacti_html_context_escape('</script><script>alert(1)</script>', CACTI_ESC_JS_STRING);
	expect($out)->not->toContain('<')
		->and($out)->not->toContain('>');
});

it('CACTI_ESC_JS_STRING — JavaScript string literal — escapes ampersand so HTML entity injection does not decode', function () {
	$out = cacti_html_context_escape('a & b', CACTI_ESC_JS_STRING);
	expect($out)->not->toContain('&')
		->and($out)->toContain('\\u0026');
});

it('CACTI_ESC_JS_STRING — JavaScript string literal — escapes newlines and carriage returns (break string literals)', function () {
...
it('CACTI_ESC_JS_STRING — JavaScript string literal — produces no surrounding quotes (caller owns delimiters)', function () {
	$out = cacti_html_context_escape('abc', CACTI_ESC_JS_STRING);
	expect(substr($out, 0, 1))->not->toBe('"');
	expect(substr($out, -1))->not->toBe('"');
});

it('CACTI_ESC_JS_STRING — JavaScript string literal — returns empty string on json_encode failure (fail closed)', function () {
...
// CACTI_ESC_URL — URL component
it('CACTI_ESC_URL — URL component — encodes reserved characters', function () {
...
it('CACTI_ESC_URL — URL component — uses %20 for space, not + (rawurlencode vs urlencode)', function () {
	$out = cacti_html_context_escape('hello world', CACTI_ESC_URL);
	expect($out)->toContain('%20')->and($out)->not->toContain('+');
});

// CACTI_ESC_CSS — CSS string value
it('CACTI_ESC_CSS — CSS string value — escapes non-alphanumeric characters as hex', function () {
...
it('CACTI_ESC_CSS — CSS string value — preserves alphanumerics, hyphen, underscore', function () {
	expect(cacti_html_context_escape('red_color-name1', CACTI_ESC_CSS))
		->toBe('red_color-name1');
});

// unknown context
it('unknown context — falls back to HTML-element escape (fail closed)', function () {
...
// type coercion
it('type coercion — stringifies non-string input', function () {
