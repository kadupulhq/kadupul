...
test('a directory swapped during candidate generation is refused before XML creation', function () {
    ...
    try {
        ...
        if (is_array($result) && is_resource($result['handle'])) { fclose($result['handle']); }
        expect($result)->toBeFalse()
            ->and(glob($root . '/outside/*'))->toBe(array());
    } finally {
        foreach (glob($root . '/outside/*') as $file) { unlink($file); }
        if (is_link($root . '/temp')) { unlink($root . '/temp'); } else { rmdir($root . '/temp'); }
        if (is_dir($root . '/held')) { rmdir($root . '/held'); }
        rmdir($root . '/outside');
        rmdir($root);
