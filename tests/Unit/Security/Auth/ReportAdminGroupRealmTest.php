...
function save_item_as($user_id, $report_id = 7, $item_id = 70)
{
...
test('report item saves require an authorized existing parent and matching item', function ($user, $report, $item, $allowed) {
    $GLOBALS['rg_db']->exec('INSERT INTO reports VALUES (8,16); INSERT INTO reports_items VALUES (80,8,1)');
    save_item_as($user, $report, $item);
    expect($GLOBALS['rg_saved'])->toBe($allowed ? array('reports_items') : array());
})->with(array(
    array(5, 7, 70, true),
    array(16, 7, 70, false),
    array(5, 7, 80, false),
...
test('report item edits require an authorized existing parent and matching item', function ($user, $report, $item, $allowed) {
    $GLOBALS['rg_db']->exec('INSERT INTO reports VALUES (8,16); INSERT INTO reports_items VALUES (80,8,1)');
    edit_item_as($user, $report, $item);
    expect($GLOBALS['rg_loaded'])->toBe($allowed ? array(array($item)) : array());
})->with(array(
    array(5, 7, 70, true),
    array(16, 7, 70, false),
    array(5, 7, 80, false),
