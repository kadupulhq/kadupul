...
namespace InventoryAccessibleLabelTest;

function __($text) { return $text; }

$baseline = json_decode(file_get_contents(__DIR__ . '/inventory-label-baseline.json'), true, 512, JSON_THROW_ON_ERROR);
$cases = array();
foreach ($baseline['issues'] as $issue) { $cases[$issue['key']] = array($issue); }
dataset('inventory accessible labels', $cases);

function render($fragment) {
	ob_start();
	try {
		eval('namespace InventoryAccessibleLabelTest; ?><table><tr><td>' . $fragment . '<option value="7" selected>7</option></select></td></tr></table>');
		return ob_get_contents();
	} finally { ob_end_clean(); }
}

test('each inventory fix maps to a unique baseline finding', function () use ($baseline) {
	expect(count($baseline['issues']))->toBe(13);
	expect(count(array_unique(array_column($baseline['issues'], 'key'))))->toBe(13);
});

test('inventory controls have associated visible names without changing other markup', function ($issue) {
...
})->with('inventory accessible labels');
