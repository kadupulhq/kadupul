...
use Kadupul\Inventory\Infrastructure\Legacy\LegacyDeviceRemovals;
...
final class DeviceRemovalWorkerTest extends TestCase
{
    ...
    {
        $directory = sys_get_temp_dir() . '/kadupul-removal-worker-' . bin2hex(random_bytes(8));
        mkdir($directory . '/bin', 0700, true);
        file_put_contents($directory . '/bin/legacy-device-remove.php', '<?php echo ' . var_export('KADUPUL_REMOVE_RESULT=' . $result, true) . ';');
        try {
            ...
            $states = new LegacyDeviceStates($database, new LegacyDeviceVisibility($database), $directory);
            $adapter = new LegacyDeviceRemovals($database, $states, $directory);
            ...
            $adapter->remove(1, $selection, DeviceRemovalPolicy::Retain);
            self::assertTrue(true);
        } finally {
            unlink($directory . '/bin/legacy-device-remove.php');
            rmdir($directory . '/bin');
            rmdir($directory);
        }
    }
