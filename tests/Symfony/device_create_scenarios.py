...
def verify_device_create(harness, session, user_id, check):
    ...
    def worker(actor, values):
        payload = base64.b64encode(json.dumps({'actor': actor, 'fields': values}).encode()).decode()
        code = '$process = proc_open([PHP_BINARY, "bin/legacy-device-create.php"], [["pipe","r"],["pipe","w"],["pipe","w"]], $pipes); fwrite($pipes[0], base64_decode("' + payload + '")); fclose($pipes[0]); $result=stream_get_contents($pipes[1]); fclose($pipes[1]); stream_get_contents($pipes[2]); fclose($pipes[2]); proc_close($process); echo $result;'
        result = harness.php('-r', code)
        return json.loads(result['stdout'].split('KADUPUL_CREATE_RESULT=')[-1])
    ...
    try:
        ...
        check(worker(user_id, command | {'site_id': '4294967295'})['status'] == 'invalid', 'worker rechecks reference existence before saving')
        ...
        try:
            check(worker(user_id, command)['status'] == 'denied', 'worker independently rechecks revoked actor permissions')
