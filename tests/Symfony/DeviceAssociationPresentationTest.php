...
final class DeviceAssociationPresentationTest extends TestCase
{
    ...
    public function testFrenchPresentationEscapesNamesAndPreservesAssignmentValues(?bool $unauthenticated, int $expectedStatus): void
    {
        ...
        try {
            ...
            $save = $port->expects(self::once())->method('change')->with(42, 7, self::callback(fn($change) => $change->targetId === 2 && $change->operation === 'add'), $device->revision());
            ...
            $path = '/inventory/devices/7/associations/graph';
            $response = $kernel->handle(Request::create($path, 'GET', [], ['Cacti' => 'fixture']));
            self::assertSame(200, $response->getStatusCode());
            self::assertStringContainsString('Associations de modèles de graphiques de l’appareil', $response->getContent());
            ...
            $fields = ['operation' => 'add', 'target' => '2', 'revision' => $device->revision(), '_token' => $token];
