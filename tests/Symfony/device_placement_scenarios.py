...
def verify_device_placement(harness, session, user_id, check):
    ...
    try:
        ...
        for kind, destination, parent in [('tree', tree, branch), ('report', report, 0)]:
            probe = harness.php('-r', f'$placementFixture = ["{kind}",{destination},{parent},{ids[0]},{user_id}];' + Path(__file__).with_name('placement_lock_probe.php').read_text().removeprefix('<?php'))
