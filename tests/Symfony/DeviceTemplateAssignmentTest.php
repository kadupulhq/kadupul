...
final class DeviceTemplateAssignmentTest extends TestCase
{
    public function testStaleAndInvalidAssignmentsDoNotMutate(): void
    {
        $device = new DeviceTemplateAssignment(7, 'Router', 1, 1);
        foreach ([-1, 16777216] as $invalid) {
            try {
                $device->assign($invalid, $device->revision());
                self::fail('Invalid template accepted');
            } catch (\InvalidArgumentException) {
                self::assertSame(1, $device->templateId());
            }
        }
        $revision = $device->revision();
        $device->assign(0, $revision);
        self::assertSame(0, $device->templateId());
        self::assertNotSame($revision, $device->revision());
        $this->expectException(DeviceEditConflict::class);
        $device->assign(2, $revision);
    }

    public function testCollectorChangesInvalidateAssignmentConfirmation(): void
    {
        $before = new DeviceTemplateAssignment(7, 'Router', 1, 1);
        $moved = new DeviceTemplateAssignment(7, 'Router', 1, 2);
        $this->expectException(DeviceEditConflict::class);
    ...
    public function testAuthorizedCommandUsesVisibleAggregateAndExpectedRevision(): void
    {
        ...
        $port->expects(self::once())->method('save')->with(42, self::callback(fn($value) => $value->templateId() === 2), $revision);
        (new AssignDeviceTemplate($access, $port))(7, 2, $revision);
    }
    ...
    public function testDeniedCommandsAndQueriesDoNotReadOrWrite(): void
    {
        foreach ([null, new Actor(42, 'operator')] as $actor) {
            ...
            $port->expects(self::never())->method('templates');
            $port->expects(self::never())->method('save');
            foreach ([fn() => (new AssignDeviceTemplate($access, $port))(7, 2, ''), fn() => (new PrepareDeviceTemplateAssignment($access, $port))(7)] as $action) {
                try {
                    $action();
    ...
    public function testMissingAndHiddenDevicesCannotBeWritten(): void
    {
        ...
        $port->expects(self::never())->method('templates');
        $port->expects(self::never())->method('save');
        self::assertSame(['device' => null, 'templates' => []], (new PrepareDeviceTemplateAssignment($access, $port))(7));
        $this->expectException(InventoryAccessDenied::class);
        (new AssignDeviceTemplate($access, $port))(7, 2, '');
    }
}
