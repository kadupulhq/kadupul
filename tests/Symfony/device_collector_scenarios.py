...
def verify_remote_collector_assignment(harness, session, device_id, poller, check):
    ...
    try:
        ...
        try:
            check(form.request()[0] == 200, 'collector form renders with a disabled current collector')
            check(form.assign(1) == 502, 'disabled source retains the online-collector requirement')
        finally:
            harness.sql(f"UPDATE poller SET disabled='' WHERE id={poller}")
        check(form.assign(1) == 200, 'collector reassignment succeeds once the source is online')
