...
def main():
    ...
    try:
        ...
        check(response['login_form'], 'legacy entry requires login')
        ...
        check(response['status'] == 401 and response.get('json') == {'error': 'authentication_required'},
              'standalone Symfony entry cannot authenticate')
        ...
        def permission_denied(session):
            ...
            return response['status'] == 200 and 'Permission Denied' in (response['title'] or '')
        ...
        check(session.request('/public/index.php/session')['status'] == 401,
              'cookie alone cannot bypass authenticated entry boundary')
        ...
        for column in ('enabled', 'locked'):
            ...
            check(actor(session)['status'] == 403, column + ' change rejects persisted session')
            restore = 'on' if column == 'enabled' else ''
            harness.sql(f"UPDATE user_auth SET {column}='{restore}' WHERE id={user_id}")
            check(actor(session)['login_form'], 're-enabling does not resurrect rejected session')
            session = login()

        session.request('/logout.php')
        check(actor(session)['login_form'], 'legacy logout removes Symfony access')
        harness.sql(f"UPDATE user_auth SET must_change_password='on', password_change='on' WHERE id={user_id}")
        session = login()
        check('/auth_changepassword.php' in actor(session)['url'],
        ...
        check(actor(session)['status'] == 403, 'deleted account cannot use persisted session')
