...
function read_user_setting($name, $default = '', $force = false) { return $default; }
function cacti_sizeof($value) { return is_array($value) ? count($value) : 0; }
function cacti_log(...$args) {}
function number_format_i18n($value, $decimals = 0) { return number_format($value, $decimals); }
function __($format, ...$args) { return $args ? vsprintf($format, $args) : $format; }
function __esc($format, ...$args) { return htmlspecialchars(__($format, ...$args), ENT_QUOTES); }
function get_execution_user() { return get_current_user(); }
