...
function form_save()
{
    if (isset_request_var('save_component_template')) {
        ...
        $rrd_step = db_fetch_cell_prepared(
            'SELECT step
        ...
        $rrd_heartbeat = db_fetch_cell_prepared(
            'SELECT heartbeat
			FROM data_source_profiles
			WHERE id = ?',
            array(get_request_var('data_source_profile_id'))
        ...
        $save3['rrd_maximum']           = form_input_validate(get_nfilter_request_var('rrd_maximum'), 'rrd_maximum', '^(?:-?(?:[0-9]+(?:\.[0-9]*)?|[0-9]*\.[0-9]+)(?:[eE][+\-]?[0-9]+)?|U|\|query_ifSpeed\|)\z', (isset_request_var('t_rrd_maximum') ? true : false), 3);
        ...
        $save3['rrd_minimum']           = form_input_validate(get_nfilter_request_var('rrd_minimum'), 'rrd_minimum', '^(?:-?(?:[0-9]+(?:\.[0-9]*)?|[0-9]*\.[0-9]+)(?:[eE][+\-]?[0-9]+)?|U)\z', (isset_request_var('t_rrd_minimum') ? true : false), 3);
        ...
        $input_fields = db_fetch_assoc_prepared(
            "SELECT id, input_output, regexp_match,
        ...
        if (!is_error_message()) {
            ...
            $data_template_fields = db_fetch_assoc_prepared(
                'SELECT
        ...
        if (!is_error_message()) {
            if (!isempty_request_var('data_template_id')) {
                ...
                db_execute_prepared(
                    'DELETE FROM poller_data_template_field_mappings
					WHERE data_template_id = ?',
                    array($data_template_id)
                );

                db_execute_prepared(
                    'REPLACE INTO poller_data_template_field_mappings
...
function form_actions()
{
    ...
    if (isset_request_var('selected_items')) {
        ...
        if ($selected_items != false) {
            if (get_nfilter_request_var('drp_action') == '1') { // delete
                ...
                $heartbeat = db_fetch_cell_prepared(
                    'SELECT heartbeat
					FROM data_source_profiles
					WHERE id = ?',
                    array(get_filter_request_var('data_source_profile_id'))
    ...
    if (isset($ds_array) && cacti_sizeof($ds_array)) {
        if (get_request_var('drp_action') == '1') { // delete
            ...
            form_text_box('title_format', '<' . __('template_title') . '> (1)', '', '255', '30', 'text');
            print "</p>
				</td>
			</tr>\n";
...
function template_rrd_remove()
{
    ...
    $children = db_fetch_assoc_prepared(
        'SELECT id
...
function template_rrd_add()
{
    ...
    if (cacti_sizeof($children)) {
        foreach ($children as $item) {
            db_execute_prepared(
                'INSERT IGNORE INTO data_template_rrd
...
function template_edit()
{
    ...
    if (!isempty_request_var('id')) {
        ...
        if (cacti_sizeof($template_data)) {
            $snmp_data = db_fetch_row_prepared(
                'SELECT *
				FROM data_input
    ...
    if (!isempty_request_var('id')) {
        $template_data_rrds = db_fetch_assoc_prepared(
    ...
    if (!isempty_request_var('view_rrd')) {
        $template_rrd = db_fetch_row_prepared(
            'SELECT *
			FROM data_template_rrd
			WHERE id = ?',
    ...
    if (empty($template_data['data_input_id'])) {
        ...
        $input_type = db_fetch_cell_prepared(
            'SELECT type_id
			FROM data_input
			WHERE id = ?',
            array($template_data['data_input_id'])
    ...
    if (!isempty_request_var('id')) {
        ...
        $fields = db_fetch_assoc_prepared(
            'SELECT *
			FROM data_input_fields
			WHERE data_input_id = ?
        ...
        if (cacti_sizeof($fields) > 0) {
            ...
            foreach ($fields as $field) {
                $data_input_data = db_fetch_row_prepared(
                    'SELECT t_value, value
