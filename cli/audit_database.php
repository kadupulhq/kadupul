...
function repair_database($run = true)
{
    ...
    if (cacti_sizeof($alters)) {
        foreach ($alters as $table => $changes) {
            $tblinfo = db_fetch_row_prepared(
                'SELECT ENGINE, SUBSTRING_INDEX(TABLE_COLLATION, "_", 1) AS COLLATION
...
function report_audit_results($output = true)
{
    ...
    if (cacti_sizeof($tables)) {
        foreach ($tables as $table) {
            ...
            $table_exists = db_fetch_cell_prepared(
                'SELECT COUNT(*)
				FROM table_columns
				WHERE table_name = ?',
                array($table_name)
            ...
            if (!$table_exists) {
                $plugin_table = db_fetch_row_prepared(
                    'SELECT *
            ...
            if ($exists) {
                if (cacti_sizeof($columns)) {
                    foreach ($columns as $c) {
                        ...
                        if (!cacti_sizeof($dbc)) {
                            $plugin_column = db_fetch_row_prepared(
                                'SELECT *
								FROM plugin_db_changes
								WHERE `table` = ?
								AND `column` = ?
                 ...
                 * missing information.
                 */
                $db_columns = db_fetch_assoc_prepared(
                    'SELECT *
					FROM table_columns
					WHERE table_name = ?',
                    array($table_name)
                ...
                if (cacti_sizeof($indexes)) {
                    foreach ($indexes as $i) {
                        $key_exists = db_fetch_cell_prepared(
                            'SELECT COUNT(*)
							FROM table_indexes
                ...
                $db_indexes = db_fetch_assoc_prepared(
                    'SELECT *
					FROM table_indexes
					WHERE idx_table_name = ?',
                    array($table_name)
                ...
                if (cacti_sizeof($db_indexes)) {
                    foreach ($db_indexes as $i) {
                        if (!db_index_exists($table_name, $i['idx_key_name'])) {
                            ...
                            $prop_seq = db_fetch_cell_prepared(
                                'SELECT COUNT(*)
								FROM table_indexes
								WHERE idx_table_name = ?
								AND idx_key_name = ?',
...
function get_previous_column($table, $column)
{
    ...
    if (!empty($sequence)) {
        if ($sequence == 1) {
            ...
            $previous = db_fetch_cell_prepared(
                'SELECT table_field
				FROM table_columns
				WHERE table_name = ?
				AND table_sequence = ?',
