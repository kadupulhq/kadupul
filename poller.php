...
$phostname = db_fetch_cell_prepared(
    'SELECT hostname
	FROM poller
	WHERE id = ?',
    array($poller_id),
...
if ($phostname == '' || $phostname == 'localhost' || $phostname == '127.0.0.1') {
    db_execute_prepared(
		...
		SET hostname = ?
		WHERE id = ?',
        array($hostname, $poller_id),
        true,
        $poller_db_cnn_id
    );
}

$dbhostname = db_fetch_cell_prepared(
    'SELECT dbhost
	FROM poller
	WHERE id = ?',
    array($poller_id),
...
$ds_needing_fixes = db_fetch_assoc_prepared(
...
$totals = db_fetch_assoc_prepared(
    'SELECT action, COUNT(*) AS totals
	FROM poller_item AS pi
	WHERE pi.poller_id = ?
	GROUP BY action',
    array($poller_id)
 ...
 */
db_execute_prepared(
    'INSERT INTO poller (id, snmp, script, server, last_status, status)
	VALUES (?, ?, ?, ?, NOW(), 1)
...
while ($poller_runs_completed < $poller_runs) {
    ...
    if ($poller_id == '1') {
        $polling_hosts = db_fetch_assoc_prepared(
     ...
     */
    $running_processes = db_fetch_cell_prepared(
    ...
    if ($poller_id == 1) {
        $issues = db_fetch_assoc_prepared(
            'SELECT ' . SQL_NO_CACHE . ' local_data_id, rrd_name
    ...
    if (read_config_option('poller_enabled') == 'on') {
        ...
        if (cacti_sizeof($polling_hosts)) {
            ...
            while (1) {
                $finished_processes = db_fetch_cell_prepared(
                    'SELECT ' . SQL_NO_CACHE . " count(*)
					FROM poller_time
					WHERE poller_id = ?
					AND end_time >'0000-00-00 00:00:00'",
        ...
        $commands = db_fetch_cell_prepared(
            'SELECT ' . SQL_NO_CACHE . ' COUNT(*)
			FROM poller_command
			WHERE poller_id = ?',
            array($poller_id),
...
function poller_heartbeat_check()
{
    ...
    $heartbeat_pollers = db_fetch_assoc_prepared(
        'SELECT *, UNIX_TIMESTAMP() - UNIX_TIMESTAMP(last_status) AS heartbeat
		FROM poller
		WHERE disabled = ""
		HAVING heartbeat > ? * 2
		OR status = 6',
...
function poller_enabled_check($poller_id)
{
    ...
    $poller_disabled = db_fetch_cell_prepared(
        'SELECT disabled
		FROM poller
		WHERE id = ?',
        array($poller_id),
