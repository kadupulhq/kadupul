...
function graphs()
{
    ...
    if (!isempty_request_var('host_id')) {
        $host   = db_fetch_row_prepared(
            'SELECT id, description, hostname, host_template_id
			FROM host
			WHERE id = ?',
    ...
    $total_rows = cacti_sizeof(db_fetch_assoc_prepared(
    ...
    if (get_request_var('graph_type') < 0) {
        ...
        if (!isempty_request_var('host_id')) {
            $template_graphs = db_fetch_assoc_prepared(
                'SELECT
            ...
            foreach ($snmp_queries as $snmp_query) {
                ...
                if (cacti_sizeof($snmp_query_graphs)) {
                    foreach ($snmp_query_graphs as $snmp_query_graph) {
                        $created_graphs = db_fetch_assoc_prepared(
                ...
                if (cacti_sizeof($xml_array)) {
                    ...
                    if (get_request_var('filter') == '' || (get_request_var('filter') != '' && cacti_sizeof($indexes))) {
                        ...
                        $field_names = db_fetch_assoc_prepared(
                ...
                $data_query_graphs = db_fetch_assoc_prepared(
                    'SELECT id, name
					FROM snmp_query_graph
					WHERE snmp_query_id = ?
					ORDER BY name',
