...
function process_poller_output_rt($rrdtool_pipe, $poller_id, $interval)
{
    ...
    $results = db_fetch_assoc_prepared(
        'SELECT port.output, port.time, port.local_data_id,
    ...
    if (cacti_sizeof($results)) {
        ...
        foreach ($results as $item) {
            ...
            if ((is_numeric($value)) || ($value == 'U')) {
                ...
                $field_rows = db_fetch_assoc_prepared(
        ...
        foreach ($results as $item) {
            ...
            if (db_execute_prepared(
                'DELETE FROM poller_output_realtime
				WHERE local_data_id = ?
				AND rrd_name = ?
				AND time = ?
				AND poller_id = ?
				AND CAST(CONVERT(output USING utf8mb4) AS BINARY) = CAST(CONVERT(? USING utf8mb4) AS BINARY)',
                array($item['local_data_id'], $item['rrd_name'], $item['time'], $poller_id, $item['output'])
            ) === false) {
                return false;
            }
        }
