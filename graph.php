			...
			<tr class='tableRowGraph'>
				<td class='center'>
					<table class='graphWrapperOuter' data-disabled='<?php print ($graph['disabled'] == 'on' ? 'true':'false');?>'>
						<tr>
							<td>
								<div class='graphWrapper' id='wrapper_<?php print htmlspecialchars((string) $graph['local_graph_id'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' graph_id='<?php print htmlspecialchars((string) $graph['local_graph_id'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' rra_id='<?php print htmlspecialchars((string) $rra['id'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' graph_width='<?php print htmlspecialchars((string) $graph['width'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' graph_height='<?php print htmlspecialchars((string) $graph['height'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' graph_start='<?php print htmlspecialchars((string) $graph_start, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' graph_end='<?php print htmlspecialchars((string) $graph_end, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' title_font_size='<?php print htmlspecialchars((string) ((read_user_setting('custom_fonts') == 'on') ? read_user_setting('title_size') : read_config_option('title_size')), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>'></div>
							</td>

							<?php if (is_realm_allowed(27)) { ?><td id='dd<?php print htmlspecialchars((string) get_request_var('local_graph_id'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' style='vertical-align:top;' class='graphDrillDown noprint'>
								<a class='iconLink utils' href='#' id='graph_<?php print htmlspecialchars((string) get_request_var('local_graph_id'), ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>_util' graph_start='<?php print htmlspecialchars((string) $graph_start, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' graph_end='<?php print htmlspecialchars((string) $graph_end, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' rra_id='<?php print htmlspecialchars((string) $rra['id'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>'><img class='drillDown' src='<?php print htmlspecialchars($config['url_path'] . 'images/cog.png', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' alt='' title='<?php print __esc('Graph Details, Zooming and Debugging Utilities');?>'></a><br>
								<a id='graph_<?php print htmlspecialchars((string) $rra['id'], ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>_csv' class='iconLink csv' href='<?php print html_escape($config['url_path'] . 'graph_xport.php?local_graph_id=' . get_request_var('local_graph_id') . '&rra_id=' . $rra['id'] . '&view_type=' . get_request_var('view_type') .  '&graph_start=' . $graph_start . '&graph_end=' . $graph_end);?>'><img src='<?php print htmlspecialchars($config['url_path'] . 'images/table_go.png', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8');?>' alt='' title='<?php print __esc('CSV Export');?>'></a><br>
								...
								if (read_config_option('realtime_enabled') == 'on' || is_realm_allowed(25)) {
									...
									print "<a class='iconLink' href='#' onclick=\"" . htmlspecialchars($popup_handler, ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "\"><img src='" . htmlspecialchars($config['url_path'] . 'images/chart_curve_go.png', ENT_QUOTES | ENT_SUBSTITUTE, 'UTF-8') . "' alt='' title='" . __esc('Click to view just this Graph in Real-time') . "'></a><br/>\n";
								}
