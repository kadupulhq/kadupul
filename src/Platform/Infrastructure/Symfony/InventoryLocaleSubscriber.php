...
final readonly class InventoryLocaleSubscriber implements EventSubscriberInterface
{
    ...
    public function onRequest(RequestEvent $event): void
    {
        ...
        if (!$event->isMainRequest() || !in_array($request->attributes->get('_route'), ['inventory_sites', 'inventory_site_edit', 'inventory_site_create', 'inventory_device_create', 'inventory_site_action', 'inventory_device_automation', 'inventory_devices_legacy', 'inventory_sites_legacy', 'inventory_devices', 'inventory_device_details', 'inventory_device_edit', 'inventory_device_template', 'inventory_device_collector', 'inventory_device_associations', 'inventory_device_maintenance', 'inventory_device_state', 'inventory_device_placement', 'inventory_device_bulk_assignment', 'inventory_device_remove'], true)) {
            return;
        }
