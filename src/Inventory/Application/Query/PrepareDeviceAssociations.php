...
final readonly class PrepareDeviceAssociations
{
    ...
    public function __invoke(int $id, string $kind): array
    {
        ...
        return ['default_reindex' => $device !== null && $kind === 'query' && $device->snmpVersion !== 0 ? $this->store->defaultReindexMethod() : 0, 'device' => $device, 'available' => $device === null ? [] : $this->store->available($kind)];
    }
}
