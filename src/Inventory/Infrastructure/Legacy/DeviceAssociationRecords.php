...
final class DeviceAssociationRecords
{
    public function snapshot(PDO $db, array $row, string $kind, bool $lock = false): DeviceAssociations
    {
        ...
        $query->execute([(int) $row['id']]);
        return new DeviceAssociations((int) $row['id'], (string) $row['description'], (int) $row['site_id'], (int) $row['poller_id'], (int) $row['host_template_id'], $query->fetchAll(PDO::FETCH_KEY_PAIR), [], 'graph', (int) $row['snmp_version']);
    }
