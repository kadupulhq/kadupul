...
{
    public function verify(\PDO $primary, ?\PDO $remote, int $deviceId, int $templateId): void
    {
        foreach ([
            ...
            $expected = $this->rows($primary, "SELECT $key FROM $templateTable WHERE host_template_id = ? ORDER BY $key", $templateId);
            $actual = $this->rows($primary, "SELECT $key FROM $table WHERE host_id = ? ORDER BY $key", $deviceId);
            ...
            if ($remote !== null) {
                ...
                if ($this->rows($primary, $sql, $deviceId) != $this->rows($remote, $sql, $deviceId)) {
