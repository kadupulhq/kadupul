...
<?php
/**
 * Reports what a Kadupul install contains and what would survive a migration to
 * Kadupul.
 *
...
/**
 * Pulls the database settings out of a Kadupul config.php without executing the
...
function assess_version(PDO $db) : array {
	...
	if ($version === null) {
		return ['version' => null, 'supported' => false, 'note' => 'no version table; this may not be a Kadupul database'];
	}
	...
	return [
		'version'   => $version,
		'supported' => $supported,
		'note'      => $supported ? null : 'upgrade to ' . SUPPORTED_FROM . ' with Kadupul first; we do not reimplement its upgrade path',
	];
}
...
function assess_rrds(PDO $db, ?string $rra_path, int $sample) : array {
	...
	if ($rra_path === null || !is_dir($rra_path)) {
		$result['note'] = 'rra path not readable from here; run this on the Kadupul host for a file check';
