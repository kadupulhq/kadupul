...
## Coding standards
- On `main`, PHP files move to PHP-FIG PER-CS 2.0 as configured in `.php-cs-fixer.php`, one file at a time. Every PHP file a change touches that the Finder includes (it excludes `include/vendor` and `tests/Fixtures`) must end up formatted; only a rename that leaves the content alone keeps the old formatting. Do not reformat files the change does not otherwise edit.
  - Convert a file in its own formatting-only commit before the behaviour change, and convert it completely. That commit changes whitespace only; token changes such as `array()` to `[]` or trailing commas go in separate commits. `tests/tools/convert_php_style.sh <file>...` makes that commit.
  - `tests/tools/check_php_style.sh` checks every changed PHP file the Finder includes; CI runs the same script. Flag a touched file that is still unconverted, and a conversion mixed into a behaviour-change commit.
