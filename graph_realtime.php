				...
				<select id='graph_start' onChange='imageOptionsChanged("timespan")'>
                    ...
                    foreach ($realtime_window as $interval => $text) {
                        printf(
                            ...
                            $interval,
                            $interval == abs(get_request_var('graph_start')) ? ' selected="selected"' : '',
                            $text
                        );
                    }
?>
...
foreach ($realtime_refresh as $interval => $text) {
    if ($interval >= $min_refresh) {
        printf(
            '<option value="%d"%s>%s</option>',
            $interval,
            $interval == get_request_var('ds_step') ? ' selected="selected"' : '',
            $text
        );
    }
}
?>
    ...
    foreach ($sizes as $key => $value) {
        printf(
            '<option value="%d"%s>%s</option>',
            $key,
            $key == get_request_var('size') ? ' selected="selected"' : '',
            $value
        );
    }
?>
