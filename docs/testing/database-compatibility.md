...
`tests/integration/PollerAcknowledgementDatabaseTest.php`, selected by `tests/phpunit-database.xml`, exercises the production acknowledgement helper on both vendors. LTS additionally runs its Boost process-table and durable migration contracts in `tests/phpunit-boost-mariadb.xml`. Tests use a dedicated disposable database and must never be pointed at production.
...
35 queue acknowledgement contracts per engine; LTS ran 25 contracts, including
...
| --- | --- | --- | --- |
| MariaDB 10.6 | 10.6.28 | 35 passed | 25 passed |
| MariaDB 10.11 | 10.11.19 | 35 passed | 25 passed |
| MariaDB 11.8 | 11.8.8 | 35 passed | 25 passed |
| MySQL 8.0 | 8.0.46 | 35 passed | 25 passed |
| MySQL 8.4 | 8.4.11 | 35 passed | 25 passed |
| MySQL 9.7 | 9.7.2 | 35 passed | 25 passed |

Boost on-demand and archive-consumer statements use the same canonical comparison.
Sixteen additional real-engine cases exercise their exact production DELETEs in
live and archive tables; native consumer tests verify the selected-row bindings.
