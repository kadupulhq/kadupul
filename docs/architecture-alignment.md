...
| Read repository ports | Transitional | Inventory queries depend on application ports. Legacy-schema adapters use prepared PDO projections. | Introduce Doctrine DBAL behind ports when it improves the adapters; application and domain APIs must not change. |
| Pinned offline browser assets | Partial | Migrated pages require no CDN and render with repository-owned Twig/CSS. Bootstrap and the planned web-component layer are not present. | Pin any introduced browser dependency and verify a clean offline build. |
