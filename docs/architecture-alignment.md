...
| Read repository ports | Partial DBAL adoption | Inventory queries depend on application ports. Assignable-site and device-creation choice reads use Doctrine DBAL behind their existing ports; permission-filtered and other legacy-schema adapters retain prepared PDO projections. The DBAL connection accepts an optional SELECT-only MySQL user and falls back to the primary credentials when none is set. | Move another read adapter only with behavior coverage; application and domain APIs must not change. |
