...
| Read repository ports | Partial DBAL adoption | Inventory queries depend on application ports. Assignable-site reads use Doctrine DBAL behind the existing port; other legacy-schema adapters retain prepared PDO projections. | Move another read adapter only with behavior coverage; application and domain APIs must not change. |
