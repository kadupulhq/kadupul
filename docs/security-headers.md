...
narrow `Content-Security-Policy` to static files. It also denies
dotfiles, dot-directories such as `.git/`, and the dependency manifests
while leaving `/.well-known/` reachable. On Apache 2.2 the dot-directory
deny needs `mod_rewrite`; without it the server answers 500 rather than
serve `.git/`.

The `.htaccess` files shipped in `cache/`, `docs/`, `include/` and `lib/`
are active wherever `AllowOverride` lets Apache read them. `docs/` keeps
serving `index.php` and the local HTML documentation with its styles,
scripts and images, so help links still work. The deny on PHP under
