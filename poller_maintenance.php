...
function remove_aged_row_cache()
{
    ...
    if (cacti_sizeof($classes)) {
        foreach ($classes as $name => $ts) {
            db_execute_prepared(
                'DELETE FROM user_auth_row_cache
...
function rrdfile_purge($force)
{
    ...
    if ($purge) {
        ...
        while (true) {
            ...
            $file_array = db_fetch_assoc_prepared(
                'SELECT DISTINCT id, name, local_data_id, action
				FROM data_source_purge_action
...
function remove_files($file_array, &$retained = 0)
{
    ...
    try {
        ...
        foreach ($file_array as $file) {
            ...
            db_execute_prepared(
                'DELETE FROM `data_source_purge_action`
			WHERE name = ?',
                array($file['name'])
            );
