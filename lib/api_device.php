            ...
            if (($rcnn_id = $reviewed_connection ?? poller_push_to_remote_db_connect($poller_id, true)) !== false) {
                ...
                if ($reviewed_associations === null) {
                    ...
                    foreach ($device_ids as $device_id) {
                        ...
                        // Device-only metadata is owned by the reviewed host. Polling
                        // and graph-bearing placements also require reviewed child IDs.
                        if (cacti_sizeof($data_sources)) {
                            db_execute('DELETE FROM poller_item WHERE host_id = ' . (int) $device_id . ' AND local_data_id IN (' . implode(', ', $data_sources) . ')', true, $rcnn_id);
                        }
                        $graph_scope = cacti_sizeof($graphs) ? ' OR local_graph_id IN (' . implode(', ', $graphs) . ')' : '';
                        foreach (array('graph_tree_items', 'reports_items') as $table) {
                            db_execute('DELETE FROM ' . $table . ' WHERE host_id = ' . (int) $device_id . ' AND (local_graph_id = 0' . $graph_scope . ')', true, $rcnn_id);
                        }
        ...
        if ($poller_id > 1) {
            ...
            if (remote_poller_up($poller_id)) {
                if (($rcnn_id = poller_push_to_remote_db_connect($device_id)) !== false) {
                    ...
                    $poller_cache = db_fetch_cell_prepared(
                        'SELECT COUNT(local_data_id)
						FROM poller_item
						WHERE host_id = ?',
                        array($device_id),
                        '',
        ...
        foreach ($fields_host_edit as $field_name => $field_array) {
            if (isset($post["t_$field_name"])) {
                if ($field_name == 'poller_id') {
                    $old_poller = db_fetch_cell_prepared(
                        'SELECT poller_id
						FROM host
						WHERE id = ?',
                        array($device_id)
...
function api_device_sync_device_templates($device_ids)
{
    ...
    foreach ($device_ids as $device_id) {
        $device_template_id = db_fetch_cell_prepared(
...
function api_device_save(
    ...
    if (empty($id)) {
        ...
        $_host_template_id = db_fetch_cell_prepared(
            'SELECT host_template_id
			FROM host
			WHERE id = ?',
            array($id)
 ...
 */
function api_clone_device_template_check_for_errors(
    $device_template_id,
    ...
    $device_template = db_fetch_row_prepared(
        'SELECT *
		FROM host_template
		WHERE id = ?',
    ...
    if ($device_template_name == '') {
        $device_template_name = db_fetch_cell_prepared(
            'SELECT name
			FROM host_template
			WHERE id = ?',
            array($device_template_id)
...
function api_clone_device_template_get_objects($device_template_id)
{
    ...
    $objects['data_queries'] = array_rekey(
        db_fetch_assoc_prepared(
            'SELECT sq.id, sq.name, sq.hash, sq.data_input_id, di.hash AS dihash,
        ...
        'id',
        array('name', 'hash', 'dihash', 'data_input_id', 'xml_path', 'input_string')
    );
    ...
    if (cacti_sizeof($objects['graph_templates'])) {
        $objects['data_templates'] = array_rekey(
            db_fetch_assoc_prepared(
                'SELECT DISTINCT dt.id, dt.name, dt.hash, dtd.data_input_id, di.hash AS dihash,
            ...
            'id',
            array('name', 'hash', 'dihash', 'data_input_id', 'input_string')
        );
        ...
        if (cacti_sizeof($objects['data_templates'])) {
            foreach ($objects['data_templates'] as $id => $data_template) {
                ...
                $graph_templates = array_rekey(
                    db_fetch_assoc_prepared(
                        'SELECT DISTINCT graph_template_id AS id
						FROM graph_templates_item AS gti
						INNER JOIN data_template_rrd AS dtr
    ...
    if (cacti_sizeof($objects['data_queries'])) {
        $objects['data_query_graph_templates'] = array_rekey(
            ...
            'id',
            array('name', 'hash', 'snmp_query_id', 'sqname')
        );
        ...
        $objects['data_query_data_templates'] = array_rekey(
            ...
            'id',
            array('name', 'hash', 'data_input_id', 'snmp_query_id')
        );
    }
 ...
 */
function api_clone_device_template(
    $template_id,
    $template_name,
    $include_gt,
    $clone_gt,
    ...
    $data_queries = db_fetch_assoc_prepared(
        "SELECT *
            ...
            foreach ($dqgt as $gt_id => $gt_data) {
                if ($gt_data['snmp_query_id'] == $id) {
                    ...
                    if ($snmp_query_graph_id > 0) {
                        ...
                        $old_snmp_query_graph_id = db_fetch_cell_prepared(
                            'SELECT id
							FROM snmp_query_graph
							WHERE snmp_query_id = ?
							AND graph_template_id = ?',
                        ...
                        $old_snmp_query_graph_rrds = db_fetch_assoc_prepared(
                            'SELECT *
							FROM snmp_query_graph_rrd
							WHERE snmp_query_graph_id = ?',
                        ...
                        $old_snmp_query_graph_rrd_sv = db_fetch_assoc_prepared(
                            'SELECT *
                        ...
                        $old_snmp_query_graph_sv = db_fetch_assoc_prepared(
                            'SELECT *
                        ...
                        if ($data_template_id > 0) {
                            $old_name = db_fetch_cell_prepared(
                                'SELECT name
								FROM data_template
								WHERE id = ?',
                            ...
                            if (!isset($duped_data_templates[$data_template_id])) {
                                ...
                                if (cacti_sizeof($old_snmp_query_graph_rrds)) {
                                    foreach ($old_snmp_query_graph_rrds as $rrd) {
                                        $data_source_name = db_fetch_cell_prepared(
                                        ...
                                        $dt_rrd_id = db_fetch_cell_prepared(
                                        ...
                                        db_execute_prepared(
                                            'INSERT INTO snmp_query_graph_rrd
