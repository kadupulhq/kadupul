...
class Installer implements JsonSerializable
{
    ...
    private function getPermissions()
    {
        ...
        if ($storage_error === '' && in_array((int) $this->getMode(), array(Installer::MODE_UPGRADE, Installer::MODE_DOWNGRADE), true)) {
            $storage_error = rrd_maintenance_queue_configuration_error($this->pollerQueueConnection());
        }
    ...
    private static function migrateProductDefaults()
    {
        global $config;

        // Normalize encodings before an exact comparison; preserve custom watermarks.
        $success = db_execute_prepared(
            'UPDATE settings SET value = ? WHERE name = ?
			AND CAST(CONVERT(value USING utf8mb4) AS BINARY) = CAST(CONVERT(? USING utf8mb4) AS BINARY)',
            array('Generated by Kadupul', 'graph_watermark', 'Generated by Cacti®')
        );

        // Outside a populated cache these can hold a scalar, and unsetting an offset
        // of a scalar is fatal. That aborted every fresh CLI install.
        if ($success) {
            if (isset($_SESSION['sess_config_array']) && is_array($_SESSION['sess_config_array'])) {
                unset($_SESSION['sess_config_array']['graph_watermark']);
            }

            if (isset($config['config_options_array']) && is_array($config['config_options_array'])) {
                unset($config['config_options_array']['graph_watermark']);
            }
        }
    ...
    private function pollerQueueConnection()
    {
        global $config, $remote_db_cnn_id;
        return (int) ($config['poller_id'] ?? 1) > 1
    ...
    private function install()
    {
        ...
        if (empty($failure) && !self::migrateProductDefaults()) {
    ...
    private function installServer()
    {
        ...
        if (!empty($automation_row)) {
            log_install_always('', __('Updating automation network (%s), mode "%s" => "%s", subnet "%s" => %s"', $automation_row['id'], $automation_row['enabled'], $this->automationMode ? 'on' : '', $automation_row['subnet_range'], $this->automationRange));
    ...
    private function upgradeDatabase()
    {
        ...
        require_once __DIR__ . '/rrd_maintenance.php';
        $queue_error = rrd_maintenance_queue_configuration_error($this->pollerQueueConnection());
    ...
    public static function beginInstall($backgroundArg, $installer = null)
    {
        ...
        try {
            ...
            $exception_error = __('Exception occurred during installation: #%s - %s', $e->getCode(), $e->getMessage());
            log_install_always('', $exception_error);

            // The CLI reads the step and its errors from this instance, not from
            // config, and exits non-zero only when the error list is non-empty
            if ($installer instanceof Installer) {
                $installer->setStep(Installer::STEP_ERROR);
                $installer->addError(Installer::STEP_INSTALL, '', $exception_error);
            }
