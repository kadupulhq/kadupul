...
function die_html_input_error($variable = '', $value = '', $message = '') {
	...
	if (isset_request_var('json')) {
		cacti_debug_backtrace('Validation Error' . ($variable != '' ? ', Variable:' . html_escape($variable):'') . ($value != '' ? ', Value:' . html_escape(cacti_redact_value($variable, $value)):'') . ', Source: ' . get_client_addr() . ', Request: ' . json_encode(cacti_redact_sensitive($_REQUEST), JSON_INVALID_UTF8_SUBSTITUTE), false);
		print json_encode(
			array(
				'status' => '500',
		...
		cacti_debug_backtrace('Validation Error' . ($variable != '' ? ', Variable:' . html_escape($variable):'') . ($value != '' ? ', Value:' . html_escape(cacti_redact_value($variable, $value)):'') . ', Source: ' . get_client_addr() . ', Request: ' . json_encode(cacti_redact_sensitive($_REQUEST), JSON_INVALID_UTF8_SUBSTITUTE), true);
