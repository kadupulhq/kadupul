...
function cacti_dns_read_name($packet, &$offset)
{
    ...
    while ($cursor < strlen($packet)) {
        ...
        if ($size > 63 || $cursor + 1 + $size > strlen($packet)) {
            return false;
        }
        $cursor++;
        if ($size === 0) {
...
{
    if (!is_string($packet) || strlen($packet) < 12 || substr($packet, 0, 2) !== $id) {
        return false;
    }
    ...
    if ($name === false || strcasecmp($name, $question_name) !== 0 ||
        substr($packet, $offset, 4) !== "\0\x0c\0\1") {
        return false;
    }
    $offset += 4;
    $aliases = array();
    ...
    while (isset($aliases[$name])) {
        if (isset($seen[$name]) || isset($results[$name])) {
            return false;
        }
        $seen[$name] = true;
...
function cacti_dns_reverse_lookup($ip, $dns, $timeout = 1000)
{
    ...
    foreach ($labels as $label) {
        $request .= chr(strlen($label)) . $label;
    }
    $request .= "\0\0\x0c\0\1";
    ...
    try {
        if (!stream_set_timeout($handle, intdiv($timeout, 1000), ($timeout % 1000) * 1000) ||
            !stream_set_blocking($handle, true) || @fwrite($handle, $request) !== strlen($request)) {
            return $ip;
        }
