...
function automation_url_utf8($url)
{
	...
	try {
		...
		// Unsupported configured encodings must still pass through the caller's JSON encoder.
		return $url;
	}

	return is_string($converted) ? $converted : $url;
}
...
function automation_get_dns_from_ip($ip, $dns, $timeout = 1000) {
	...
	return cacti_dns_reverse_lookup($ip, $dns, $timeout);
}
