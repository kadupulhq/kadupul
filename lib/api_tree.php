...
function api_tree_item_save(
    ...
    try {
        ...
        $result = api_tree_item_save_locked(
            $id,
            $tree_id,
            $type,
            $parent_tree_item_id,
...
function api_tree_item_save_locked(
    $id,
    $tree_id,
    $type,
    $parent_tree_item_id,
...
{
    $tree_item = db_fetch_row_prepared(
        'SELECT title, local_graph_id, site_id, host_id
		FROM graph_tree_items
		WHERE id = ?',
        array($tree_item_id)
    );
...
function api_tree_sort_branch($leaf_id, $tree_id = 0, $lock = true)
{
    ...
    if ($parent > 0) {
        ...
        $sort_style = db_fetch_cell_prepared(
            'SELECT sort_type
			FROM graph_tree
			WHERE id = ?',
            array($tree_id)
    ...
    if (cacti_sizeof($sort_array)) {
        ...
        foreach ($sort_array as $id => $element) {
            $sort = db_fetch_cell_prepared(
                'SELECT sort_children_type
				FROM graph_tree_items
				WHERE id = ?',
                array($id)
            );
    ...
    if (cacti_sizeof($sort_array)) {
        ...
        foreach ($sort_array as $id => $element) {
            ...
            if ($sort == TREE_ORDERING_INHERIT) {
                $first_child = db_fetch_cell_prepared(
                    'SELECT id
					FROM graph_tree_items
					WHERE parent = ?
					ORDER BY position
