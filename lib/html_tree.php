...
function grow_dropdown_tree($tree_id, $parent = 0, $form_name = '', $selected_tree_item_id = '', $tier = 0)
{
    ...
    $branches = db_fetch_assoc_prepared(
        'SELECT gti.id, gti.title, parent
...
function grow_dhtml_trees()
{
    ...
    if (empty($default_tree_id)) {
        ...
        if ($user['policy_trees'] == 1) {
            $default_tree_id = db_fetch_cell_prepared(
                'SELECT graph_tree.id
				FROM graph_tree
	...
	function checkTreeForLogout() {
		html = $('#jstree').html();
		found = html.indexOf('<?php print __('Login to Kadupul');?>');
		if (found >= 0) {
...
function get_tree_path()
{
    if (isset_request_var('node')) {
        ...
        if (strpos(get_request_var('node'), 'tbranch') !== false) {
            ...
            $linknode = db_fetch_row_prepared(
                'SELECT *
...
function create_branch($leaf)
{
    ...
    $children = db_fetch_cell_prepared(
        'SELECT COUNT(*)
		FROM graph_tree_items
		WHERE parent = ?
...
function grow_right_pane_tree($tree_id, $leaf_id, $host_group_data)
{
    ...
    if (($leaf_type == 'site' || $leaf_type == 'host') && $leaf['parent'] != 0) {
        ...
        while ($parent != 0) {
            $pleaf = db_fetch_row_prepared(
                'SELECT * FROM graph_tree_items
				WHERE id = ?',
                array($parent)
            );
    ...
    if (!empty($leaf_id)) {
        $host_name = db_fetch_cell_prepared(
            'SELECT description
			FROM host
			WHERE id = ?',
            array($leaf['host_id'])
        ...
        $site_name = db_fetch_cell_prepared(
            'SELECT name
			FROM sites
			WHERE id = ?',
            array($leaf['site_id'])
    ...
    if (isset_request_var('host_template_id') && get_request_var('host_template_id') > 0) {
        $host_template_name = db_fetch_cell_prepared(
            'SELECT name
			FROM host_template
			WHERE id = ?',
