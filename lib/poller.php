...
function update_reindex_cache($host_id, $data_query_id)
{
    ...
    $data_query = db_fetch_row_prepared(
        'SELECT ' . SQL_NO_CACHE . ' *
    ...
    $data_query_type = db_fetch_cell_prepared(
        'SELECT ' . SQL_NO_CACHE . ' data_input.type_id
		FROM data_input
    ...
    if (cacti_sizeof($data_query)) {
        switch ($data_query['reindex_method']) {
            ...
            case DATA_QUERY_AUTOINDEX_INDEX_NUM_CHANGE:
                 ...
                 */
                $assert_value = cacti_sizeof(db_fetch_assoc_prepared(
                    'SELECT DISTINCT ' . SQL_NO_CACHE . ' snmp_index
            ...
            case DATA_QUERY_AUTOINDEX_FIELD_VERIFICATION:
                $primary_indexes = db_fetch_assoc_prepared(
                    'SELECT ' . SQL_NO_CACHE . ' snmp_index, oid, field_value
				FROM host_snmp_cache
				WHERE host_id = ?
				AND snmp_query_id = ?
				AND field_name = ?',
...
function process_poller_output_page(&$rrdtool_pipe, $remainder, $after, &$acknowledged, &$blocked_paths, &$next)
{
    ...
    if (cacti_sizeof($results)) {
                        ...
                        if (isset($rrd_field_names[$item['data_template_id'] . '_' . $matches[0]])) {
                            ...
                            if ($data_template_id > 0) {
                                $nt_rrd_field_names = array_rekey(
                                    db_fetch_assoc_prepared(
                                        'SELECT DISTINCT dtr.data_source_name, dif.data_name
        ...
        if ($direct_update) {
            ...
            $rrds_processed = array_sum(array_map(function ($samples) {
                return count(array_filter($samples));
            }, $completed));
            $output_keys = array();
            foreach ($results as $item) {
        ...
        if (cacti_sizeof($cache)) {
            foreach ($cache as $item) {
                if (!file_exists($item['path'])) {
                    db_execute_prepared(
                        'DELETE FROM poller_resource_cache
						WHERE `path` = ?',
                        array($item['path'])
                    );
                }
        ...
        while (($entry = $pobject->read()) !== false) {
            if (!should_ignore_from_replication($entry)) {
                ...
                if (is_dir($path . DIRECTORY_SEPARATOR . $entry)) {
                    ...
                    $save         = array();
                    $save['path'] = $spath;
                    $save['id']   = db_fetch_cell_prepared(
                        'SELECT id
						FROM poller_resource_cache
						WHERE `path` = ?',
                        array($save['path'])
...
function resource_cache_out($type, $path)
{
    ...
    if (empty($last_md5) || $last_md5 != $curr_md5) {
        $entries = db_fetch_assoc_prepared(
            'SELECT id, `path`, md5sum, attributes
			FROM poller_resource_cache
			WHERE resource_type = ?',
            array($type),
            true,
            ...
            foreach ($entries as $e) {
                ...
                if (is_dir(dirname($mypath))) {
                    if ($md5sum != $e['md5sum'] && $e['path'] != 'include/config.php') {
                        ...
                        $exit = -1;
                        $contents = base64_decode(db_fetch_cell_prepared(
                            'SELECT contents
							FROM poller_resource_cache
							WHERE id = ?',
                            array($e['id']),
...
function poller_push_reindex_data_to_poller($device_id = 0, $data_query_id = 0, $force = false, $db_cnn_id = false)
{
    ...
    $min_reindex_cache = db_fetch_cell_prepared(
        "SELECT MIN(UNIX_TIMESTAMP(last_updated)-3600)
		FROM host_snmp_cache
		$sql_where",
        $sql_params
    );
    ...
    if (cacti_sizeof($recache_hosts)) {
        ...
        $local_graph_ids = db_fetch_assoc_prepared(
            "SELECT *
			FROM graph_local
			$sql_where1",
            $sql_params1
        ...
        $host_snmp_cache = db_fetch_assoc_prepared(
            "SELECT *
			FROM host_snmp_cache
			$sql_where1",
            $sql_params1
        ...
        $poller_reindex = db_fetch_assoc_prepared(
            "SELECT *
			FROM poller_reindex
			$sql_where1",
            $sql_params1
...
function poller_push_data_to_main()
{
    ...
    if ($config['poller_id'] > 1) {
        $host_records = db_fetch_assoc_prepared(
            'SELECT id, snmp_sysDescr, snmp_sysObjectID,
        ...
        $poller_item_records = db_fetch_assoc_prepared(
            'SELECT local_data_id, host_id, rrd_name, rrd_step, rrd_next_step
			FROM poller_item
			WHERE poller_id = ?',
            array($config['poller_id'])
        );
...
function remote_poller_up($poller_id)
{
    ...
    $up_poller = db_fetch_cell_prepared(
        'SELECT COUNT(*)
		FROM poller
...
function register_process($tasktype, $taskname, $taskid, $pid, $timeout)
{
    ...
    db_execute_prepared(
        'INSERT INTO processes (tasktype, taskname, taskid, pid, timeout, started, last_update)
		VALUES (?, ?, ?, ?, ?, NOW(), NOW())',
...
function process_poller_output_batch(&$deferred, &$proxy_pipe, $final = false, $deadline = null)
{
    global $config;
