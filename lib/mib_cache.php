...
class MibCache
{
        ...
        if (isset($mp->mib) && isset($mp->oids) && $mp->mib) {
            ...
            foreach ($mp->oids as $object_name => $object_params) {
                if ($object_params['otype'] != 'TEXTUAL-CONVENTION') {
                    db_execute_prepared(
                        'INSERT IGNORE INTO `snmpagent_cache`
                    ...
                    db_execute_prepared(
                        'INSERT INTO `snmpagent_cache_textual_conventions`
    ...
    public function table($table)
    {
        if ($this->active_table != $table) {
            if (!isset($this->cache__tables[$this->active_mib][$table])) {
                $oid_table = db_fetch_cell_prepared(
                    'SELECT oid
    ...
    private function exists()
    {
        ...
        $exists = db_fetch_cell_prepared(
            'SELECT 1 FROM `snmpagent_cache` WHERE `oid` = ?',
