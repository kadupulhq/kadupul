...
function utilities_view_snmp_cache()
{
    ...
    ?>
					...
					<td>
						<input type='checkbox' id='with_index' title='<?php print __esc('Allow the search term to include the index column');?>' <?php if (get_request_var('with_index') == 1) {
						    print ' checked ';
						}?>>
    ...
    if ($config['connection'] == 'online') {
        ...
        if (cacti_sizeof($runtimes)) {
            foreach ($runtimes as $r) {
                ...
                $rows_to_process = db_fetch_cell_prepared(
                    'SELECT COUNT(*)
                ...
                $runtime = db_fetch_cell_prepared(
                    'SELECT UNIX_TIMESTAMP() - UNIX_TIMESTAMP(started)
					FROM processes
